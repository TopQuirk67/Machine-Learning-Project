...
For graders, I have measured the number of words in this document to be 1161 excluding code output.  Thanks for grading!
...
- Parameters.  Once we have determined the likely best algorithm (i.e. the most accurate), we may tune is execution to arrive at the best possible results.  In our case we will briefly explore the use of PCA pre-processing.
...
While some of the variables are indeed highly correlated, there is nonetheless structure in nearly every relationship.  Even the highest correlation (`r round(max(cormtx2),4)`), between *roll_belt* and *accel_belt_z*, is incomplete.  If our purpose were to gain efficiency, it might be useful to reduce dimensionality via PCA analysis, but instead we retain all the variables, at least for the initial model formulation.  
